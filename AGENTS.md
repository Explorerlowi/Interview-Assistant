...
* 包名：
  ...
  * 根包统一：`com.example.interviewassistant`（按真实项目替换）；
  * feature 包结构：
...
shared/
 └── src/
     └── commonMain/
         └── kotlin/com/example/interviewassistant/
             ├── core/
...
shared/
 └── src/
     ├── androidMain/
     │   └── kotlin/com/example/interviewassistant/core/i18n/
...
* 包名根目录：`com.example.interviewassistant.android`
* 按功能拆包（对应 shared 的 feature）：

```text
androidApp/
 └── src/main/
     ├── java/com/example/interviewassistant/android/
     │   ├── App.kt
     │   ├── navigation/
     │   ├── core/ui/
     │   └── feature/
