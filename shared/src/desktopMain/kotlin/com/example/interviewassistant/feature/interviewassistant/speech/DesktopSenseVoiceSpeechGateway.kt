...
/** Windows sherpa-onnx implementation of VAD-assisted simulated-streaming SenseVoice recognition. */
class DesktopSenseVoiceSpeechGateway(
    private val modelManager: DesktopSenseVoiceModelManager,
    ...
    private fun createRuntime(key: RuntimeKey): Runtime {
        ...
        val senseVoiceConfig = OfflineSenseVoiceModelConfig.builder()
            .setModel(files.model)
            .setLanguage(key.language)
            .setInverseTextNormalization(key.useInverseTextNormalization)
            .build()
        val modelConfig = OfflineModelConfig.builder()
            .setSenseVoice(senseVoiceConfig)
            .setTokens(files.tokens)
            .setNumThreads(RECOGNIZER_THREADS)
            .setDebug(false)
            .setProvider("cpu")
            .build()
        ...
        val sileroVadConfig = SileroVadModelConfig.builder()
            .setModel(files.vad)
            .setThreshold(VAD_THRESHOLD)
            .setMinSilenceDuration(MIN_SILENCE_SECONDS)
            .setMinSpeechDuration(MIN_SPEECH_SECONDS)
            .setWindowSize(VAD_WINDOW_SAMPLES)
            .setMaxSpeechDuration(key.maxSpeechDurationSeconds.toFloat())
            .build()
        val vadConfig = VadModelConfig.builder()
            .setSileroVadModelConfig(sileroVadConfig)
            .setSampleRate(AUDIO_SAMPLE_RATE)
            .setNumThreads(VAD_THREADS)
            .setDebug(false)
            .setProvider("cpu")
            .build()
