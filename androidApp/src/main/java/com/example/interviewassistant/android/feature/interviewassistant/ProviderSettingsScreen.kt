...
fun ProviderSettingsScreen(
    ...
    var pendingSpeechTestMode by remember {
        mutableStateOf(configuration.speechRecognitionMode)
    }
    var pendingSenseVoiceConfiguration by remember {
        mutableStateOf(configuration.senseVoice)
    }
        ...
        if (granted) {
            onEvent(
                ProviderSettingsUiEvent.StartSpeechRecognitionTest(
                    mode = pendingSpeechTestMode,
                    senseVoiceConfiguration = pendingSenseVoiceConfiguration,
                ),
            )
        }
    }
    ...
    DisposableEffect(Unit) {
        onDispose { onEvent(ProviderSettingsUiEvent.StopSpeechRecognitionTest) }
    }
    ...
    LazyColumn(
        ...
        item {
            SettingsSection(title = strings.get(AppStringId.SETTINGS_SPEECH_RECOGNITION)) {
                Row(
                    ...
                    SpeechModeOption(
                        selected = configuration.speechRecognitionMode == SpeechRecognitionMode.SENSE_VOICE_ON_DEVICE,
                ...
                SpeechRecognitionTest(
                    ...
                    onStart = {
                        ...
                        pendingSpeechTestMode = configuration.speechRecognitionMode
                        pendingSenseVoiceConfiguration = configuration.senseVoice
...
private fun SpeechRecognitionTest(
    ...
    Surface(
        ...
        Column(
            ...
            if (permissionDenied) {
                Text(
                    text = strings.get(AppStringId.ERROR_PERMISSION_MICROPHONE),
...
private fun String?.localizedMetadata(
    ...
    if (this.isNullOrBlank()) return strings.get(AppStringId.SETTINGS_RESULT_PENDING)
...
@Composable
private fun RowScope.SpeechModeOption(
    selected: Boolean,
    label: String,
    onClick: () -> Unit,
) {
